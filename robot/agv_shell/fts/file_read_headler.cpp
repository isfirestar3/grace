...
int file_read_headler::get_fts_current_catalog(const std::string& current_path, std::vector<file::st_file_info>& vct_f)
{
#ifdef _WIN32
	//_finddata_t f_info;
	...
	do
	{
		file::st_file_info info;
		info.file_name = FindFileData.cFileName;
		info.file_attri = FindFileData.dwFileAttributes;

		//计算crc32校验码,计算文件名称+文件最近修改时间组成的字符串的crc32
		info.file_access_time = (info.file_access_time = FindFileData.ftLastAccessTime.dwHighDateTime) << 32 | FindFileData.ftLastAccessTime.dwLowDateTime;
		info.file_create_time = (info.file_create_time = FindFileData.ftCreationTime.dwHighDateTime) << 32 | FindFileData.ftCreationTime.dwLowDateTime;
		info.file_modify_time = (info.file_modify_time = FindFileData.ftLastWriteTime.dwHighDateTime) << 32 | FindFileData.ftLastWriteTime.dwLowDateTime;
		info.total_size = (info.total_size = FindFileData.nFileSizeHigh) << 32 | FindFileData.nFileSizeLow;
		char file_crc[MAX_DIR_PATH_LEN];
		sprintf_s(file_crc, MAX_DIR_PATH_LEN, "%s%I64u", info.file_name.c_str(), info.file_modify_time);
		std::string tmp = file_crc;
		info.file_crc32 = nsp::toolkit::crc32(info.file_crc32, (unsigned char*)file_crc, tmp.size());
		...
		//		char file_crc[MAX_DIR_PATH_LEN];
		//		sprintf_s(file_crc, MAX_DIR_PATH_LEN, "%s%I64u", info.file_name_.c_str(), info.file_modify_time_);
		//		std::string tmp = file_crc;
	...
	while (NULL != (ent = readdir(dirptr))) {
		...
		if (stat(filename, &st) >= 0) {
			...
			info.file_name = ent->d_name;
			info.total_size = st.st_size;
			//windows 是从16010101000000开始的秒数, 单位为100 ns, linux 是从19700101000000 开始的秒数, 相差 11644444800 s
			info.file_access_time = (st.st_atime + 11644444800 + time_zone_ * 60 * 60) * 10000000L;
			info.file_create_time = (st.st_ctime + 11644444800 + time_zone_ * 60 * 60) * 10000000L;
			info.file_modify_time = (st.st_mtime + 11644444800 + time_zone_ * 60 * 60) * 10000000L;
			...
			if (mode & __S_IFDIR) {
				...
				char file_crc[MAX_DIR_PATH_LEN];
				sprintf(file_crc, "%s%lu", info.file_name.c_str(), info.file_modify_time); //文件名+修改时间 
				std::string tmp = file_crc;
				info.file_crc32 = nsp::toolkit::crc32(info.file_crc32, (unsigned char*)file_crc, tmp.size());
				//根据内容计算CRC32 
