...
int net_manager::write(uint16_t pkt_id, const std::function<int()> &sender, const std::shared_ptr<motion::asio_block>&asio_parnet)
{
...
int net_manager::move_asio_object(uint16_t pkt_id, std::shared_ptr<motion::asio_block> &sptr)
{
	//return asio_manager_.move_asio_object(pkt_id, sptr);
	return 0;
}

int net_manager::exec(uint16_t pkt_id, const char* byte_stream)
{
...
void net_manager::build_error_notify(uint16_t pkt_id)
{
