...
//#ifdef _DEBUG
							nspwarn << "asio map data forward manage get a timeout,the cmd is " << iter->first << " ,the timeout interval:" << tick - iter->second->tick_;
//#endif
							//回调上层
...
//#ifdef _DEBUG
						nspwarn << "asio map data forward manage get a overflow timeout,the cmd is " << iter->first << " ,the timeout interval:" << tick + (MAXDWORD - iter->second->tick_);
//#endif
						//回调上层
						iter->second->error_ = nsp::proto::errorno_t::kRequestTimeout;
						timeout_notify(iter->second);
...
//#ifdef _DEBUG
							nspwarn << "asio manage get a timeout,the pkt_id is " << iter->first << " ,the timeout interval:" << tick - iter->second->tick_;
//#endif
							//回调上层
							iter->second->error_ = nsp::proto::errorno_t::kRequestTimeout;
							timeout_notify(iter->second);
...
//#ifdef _DEBUG
						nspwarn << "asio manage get a overflow timeout,the pkt_id is " << iter->first << " ,the timeout interval:" << tick + (MAXDWORD - iter->second->tick_);
//#endif
						//回调上层
						iter->second->error_ = nsp::proto::errorno_t::kRequestTimeout;
						timeout_notify(iter->second);
