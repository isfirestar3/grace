		...
		namespace udp{
			...
			struct udp_head : public nsp::proto::proto_interface{
				...
				udp_head(const udp_head&lref){
					if (&lref != this){
						...
						cmd_ = lref.cmd_;
						memcpy(reserver_second_, lref.reserver_second_, sizeof(lref.reserver_second_));
				...
				unsigned char *serialize(unsigned char*bytes)const
				{
					...
					memcpy(bytes, &sequence_id_, sizeof(sequence_id_));
					bytes += sizeof(sequence_id_);
					memcpy(bytes, &version_, sizeof(version_));
					bytes += sizeof(version_);
					memcpy(bytes, &reserver_first_, sizeof(reserver_first_));
					bytes += sizeof(reserver_first_);
					memcpy(bytes, &cmd_, sizeof(cmd_));
					bytes += sizeof(cmd_);
					memcpy(bytes, reserver_second_, sizeof(reserver_second_));
					bytes += sizeof(reserver_second_);
					memcpy(bytes, &operate_, sizeof(operate_));
					bytes += sizeof(operate_);
					memcpy(bytes, &sub_operate_, sizeof(sub_operate_));
					bytes += sizeof(sub_operate_);
					memcpy(bytes, &operate_parament_, sizeof(operate_parament_));
				...
				const unsigned char *build(const unsigned char *bytes, int &cb)
				{
					...
					memcpy(&sequence_id_, bytes, sizeof(sequence_id_));
					bytes += sizeof(sequence_id_);
					cb -= sizeof(sequence_id_);

					memcpy(&version_, bytes, sizeof(version_));
					bytes += sizeof(version_);
					cb -= sizeof(version_);

					memcpy(&reserver_first_, bytes, sizeof(reserver_first_));
					bytes += sizeof(reserver_first_);
					cb -= sizeof(reserver_first_);

					memcpy(&cmd_, bytes, sizeof(cmd_));
					bytes += sizeof(cmd_);
					cb -= sizeof(cmd_);

					memcpy(&reserver_second_, bytes, sizeof(reserver_second_));
					bytes += sizeof(reserver_second_);
					cb -= sizeof(reserver_second_);

					memcpy(&operate_, bytes, sizeof(operate_));
					bytes += sizeof(operate_);
					cb -= sizeof(operate_);

					memcpy(&sub_operate_, bytes, sizeof(sub_operate_));
					bytes += sizeof(sub_operate_);
					cb -= sizeof(sub_operate_);

					memcpy(&operate_parament_, bytes, sizeof(operate_parament_));
					bytes += sizeof(operate_parament_);
			...
			struct udp_ack_head : public nsp::proto::proto_interface{
				...
				const unsigned char *build(const unsigned char *byte_stream, int &cb)
				{
					...
					memcpy(&sequence_id_,  byte_stream, sizeof(sequence_id_));
					byte_stream += sizeof(sequence_id_);
					cb -= sizeof(sequence_id_);
					memcpy(&version_,  byte_stream, sizeof(version_));
					byte_stream += sizeof(version_);
					cb -= sizeof(version_);
					memcpy(&reserver_first_, byte_stream, sizeof(reserver_first_));
					byte_stream += sizeof(reserver_first_);
					cb -= sizeof(reserver_first_);
					memcpy(&cmd_, byte_stream, sizeof(cmd_));
					byte_stream += sizeof(cmd_);
					cb -= sizeof(cmd_);
					memcpy(&ack_, byte_stream, sizeof(ack_));
					byte_stream += sizeof(ack_);
					cb -= sizeof(ack_);
					memcpy(&reserver_second_, byte_stream, sizeof(reserver_second_));
					byte_stream += sizeof(reserver_second_);
					cb -= sizeof(reserver_second_);
					memcpy(&operate_, byte_stream, sizeof(operate_));
					byte_stream += sizeof(operate_);
					cb -= sizeof(operate_);
					memcpy(&sub_operate_, byte_stream, sizeof(sub_operate_));
					byte_stream += sizeof(sub_operate_);
					cb -= sizeof(sub_operate_);
					memcpy(&operate_parament_, byte_stream, sizeof(operate_parament_));
