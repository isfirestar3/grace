		...
		#pragma pack(push, 1)
			typedef struct __common_data : public nsp::proto::proto_interface{
				...
				const unsigned char *build(const unsigned char *bytes, int &cb)
				{
					...
					memcpy(&data_len_, pos, sizeof(data_len_));
			...
			typedef struct __can_data : public nsp::proto::proto_interface{
				...
				const int length() const
				{
					return sizeof(can_cmd) + sizeof(can_serial_index_0) + sizeof(can_serial_index_1) + sizeof(can_node_id) + 
						sizeof(data_length) + data_length*sizeof(uint8_t);
				}
