...
static int image_decompress(uint8_t *jpeg, int len, uint8_t *bmp) {
    int retval;
    jpeg_init();
    retval = jpeg_decompress(jpeg, len, bmp);
    jpeg_exit();
    return retval;
}
...
void *routine_handler(void *p) {
    ...
    while (1) {
        ts0 = ts1 = ts2 = 0;
        retval = posix__waitfor_waitable_handle(&ncb->evt_image_handler, 1000);
        if (0 == retval) {
            posix__block_waitable_handle(&ncb->evt_image_handler);
        }else if (retval < 0) {
            break;
        }
