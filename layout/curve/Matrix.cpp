...
//////////////////////////////////////////////////////////////////////
// 设置矩阵指定分块的值(调用Fortran专用)
//
// 参数：
// 1. double value[] - 一维数组，长度为m_nNumColumns*m_nNumRows，存储
//                     矩阵各元素的值
// 2. int nRows - 需要覆盖的矩阵的行数
// 3. int nCols - 需要覆盖的矩阵的列数
// 返回值：无
//////////////////////////////////////////////////////////////////////
...
//
// 参数：
// 1. const CMatrix& other - 与指定矩阵相拼接的矩阵
//
// 返回值：CMatrix型，指定矩阵与other相拼加之结果
//////////////////////////////////////////////////////////////////////
CMatrix	CMatrix::operator|(const CMatrix& other) const
{
	// 首先检查行列数是否相等
...
// 输出显示
bool CMatrix::print(double a_init[6])
{
	int i,j;
	for(i=0; i< m_nNumRows; i++){
		for(j = 0; j < m_nNumColumns; j++){
			a_init[j]=GetElement(i, j);      // 2次
		}
	}
	return false;
}
...
{
   if(row1>row2||col1>col2) return false;//行列参数有误
   if((row2-row1+1)!=other.m_nNumRows||(col2-col1+1)!=other.m_nNumColumns) return false;//行列参数与other阵的行列数不配套
   CMatrix origal(*this);//备份原矩阵
   if(row2>=m_nNumRows) m_nNumRows = row2+1;//扩展行数
   if(col2>=m_nNumColumns) m_nNumColumns=col2+1;//扩展列数
   if (m_pData)
   {	delete[] m_pData;		m_pData = NULL;	}//delete内存
   int nSize = m_nNumColumns*m_nNumRows;
   if (nSize < 0) return false;
	// 分配内存
	m_pData = new double[nSize];
	if (m_pData == NULL)	return false;	// 内存分配失败
	//重新赋值
	for(int i=0;i<m_nNumRows;i++)
		for(int j=0;j<m_nNumColumns;j++)
		{
			if(i>=row1&&i<=row2&&j>=col1&&j<=col2) m_pData[i*m_nNumColumns+j]=other.GetElement(i-row1,j-col1);//用other阵赋值相应块
		    else if((i<origal.m_nNumRows)&&(j<origal.m_nNumColumns))  m_pData[i*m_nNumColumns+j]=origal.GetElement(i,j);//原矩阵未被覆盖的块
			else   m_pData[i*m_nNumColumns+j]=0;//扩展的真空，全赋值0
		}
    return true;
}
bool CMatrix::CMsqrt(CMatrix& result)
{
	...
	if(eigenvalue==NULL) return false;//动态创建数组失败
	CMatrix eigenvector(size);
    if(JacobiEigenv2(eigenvalue,eigenvector,0.000001)==false) return false;//得到特征值和特征向量,JacobiEigenv2函数会把原矩阵变为相应的对角阵
    CMatrix symM(size);
