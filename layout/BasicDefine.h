...
typedef struct NodeInfo
{
	...
	double pos_x = 0.0;
	double pos_y = 0.0;
	int spin = 0;//是否允许旋转，0表示不允许，1表示允许
	int tshaped = 0;//是否丁字路口，0不是，1是
...
typedef struct WopAttr
{
	...
	WopAttr& operator = (const WopAttr& other)
	{
		if (this != &other)
		{
			id = other.id;
...
typedef struct EdgeInfo
{
	...
	double reLength = -100;//搜路时需要的边长度为0，-1表示还用length
	...
	std::vector<WOPATTR> wop_attr_list; //wop对应的属性值
	EdgeInfo()
	{
		EDGESPEED edgeSpeed;
...
struct PolygonVertex { // add by fengq

	double x;
	double y;
};

typedef struct Polygon { // add by fengq
...
typedef struct LayoutData
{
	...
	std::vector <POLYGON> polygonList; // add by fengq
...
typedef struct CodePosXya
{
	int pos_id = 0;
	double pos_x = 0.0;
	double pos_y = 0.0;
	double angle = 0.0;
	CodePosXya& operator = (const CodePosXya& other)
	{
		if (this == &other)
		{
			return *this;
