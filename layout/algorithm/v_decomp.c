...
void EnableDrive_unit(const list_entry_t *drive_unit, int aa) {
    ...
    for (pos = drive_unit->flink_; pos != (list_entry_t*) & drive_unit->flink_; pos = pos->flink_) {
        drive_obj = containing_record(pos, var__functional_object_t, local_);
        dri = var__object_body_ptr(var__drive_unit_t, drive_obj);
        wheel_obj = posix__malloc(dri->associated_cnt_ * sizeof ( void *));
        ...
        posix__free(wheel_obj);
    }
}

int get_num_of_wheel(const list_entry_t *drive_unit, int *num_time_stamp) {
    list_entry_t *pos;
    int num_wheel = 0;
    (*num_time_stamp) = 0;
    var__drive_unit_t *dri;
...
int chassis_state_checked(const list_entry_t *drive_unit, var__vehicle_t *veh)
{
...
void DoOptimize(double *Xresult, int N_wh, v_wheel* vwheels, double* Xinit, char *pval) {
    *pval = 0;
    ...
    double *pos_now = (double*) posix__malloc(sizeof ( double)*N_wh1);
    for (i = 0; i < N_wh1; i++) {
        *(pos_now + i) = *(Xinit + i);
    }


    double *pos_last = (double*) posix__malloc(sizeof ( double)*N_wh1);
    double *E_axis_temp = (double*) posix__malloc(sizeof ( double)*N_wh1);
    double *pos_end = (double*) posix__malloc(sizeof ( double)*N_wh1);
    ...
    double **spos_axis;
    spos_axis = (double**) posix__malloc(sizeof ( double*)*(N_wh1 + 1));
    for (i = 0; i < N_wh1 + 1; i++) {
        spos_axis[i] = (double*) posix__malloc(sizeof ( double)*N_wh1);
        memset(spos_axis[i], 0, sizeof ( double)*N_wh1);
    }

    double **E_axis;
    E_axis = (double**) posix__malloc(sizeof ( double*)*N_wh1);
    for (i = 0; i < N_wh1; i++) {
        E_axis[i] = (double*) posix__malloc(sizeof ( double)*N_wh1);
        memset(E_axis[i], 0, sizeof ( double)*N_wh1);
    }

    double *posaxis_inc = (double*) posix__malloc(sizeof ( double)*N_wh1);
    memset(posaxis_inc, 0, sizeof ( double)*N_wh1);
    double *posaxis_dec = (double*) posix__malloc(sizeof ( double)*N_wh1);
    ...
    double *datas;
    datas = (double *) posix__malloc(sizeof ( double)* N_wh1); 
    ...
    posix__free(pos_now);
    posix__free(pos_last);
    posix__free(E_axis_temp);
    posix__free(pos_end);
    for (i = 0; i < N_wh1 + 1; i++) {
        posix__free(spos_axis[i]);
    }
    posix__free(spos_axis);
    for (i = 0; i < N_wh1; i++) {
        posix__free(E_axis[i]);
    }
    posix__free(E_axis);
    posix__free(posaxis_inc);
    posix__free(posaxis_dec);
}
...
int VwResolution(v_chassis *V_com, var__vehicle_t *veh, const list_entry_t *drive_unit) {
    ...
    v_wheel * v_wheels = (v_wheel*) posix__malloc(sizeof (v_wheel) * num_wh);
    if (sdd_firsttime == 0) {
        w_sdd_last = (double*) posix__malloc(sizeof (double)*num_wh);
        memset(w_sdd_last, 0, sizeof (double)*num_wh);
    }
    first_v_decomposite(v_wheels, drive_unit, veh);
    double * va_init = (double*) posix__malloc(sizeof ( double)*(num_wh + 3));
...
int InvertGaussJordan(double *m_pData, int m_nNumColumns)
{
    ...
    pnRow = (int*) posix__malloc(sizeof ( int)*m_nNumColumns);
    pnCol = (int*) posix__malloc(sizeof ( int)*m_nNumColumns);
...
int v_composition(v_wheel * v_wheels, int num_wheel, int ref_enc, v_chassis *out)
{
    uint64_t sum_time = 0;
    int num_v = num_wheel;
    double *A = (double*) posix__malloc(sizeof (double)* 9);
    memset(A, 0, sizeof (double)* 9);
    double *B = (double*) posix__malloc(sizeof (double)* 3);
