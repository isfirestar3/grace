...
#pragma pack(push, 1)
			typedef struct __can_datapacket : public nsp::proto::proto_interface{
				uint8_t can_cmd;
				...
				__can_datapacket(uint8_t cmd, uint16_t serial_index, uint8_t node_id)
				{
					can_cmd = cmd;
					can_serial_index_0 = serial_index & 0x00ff;
					can_serial_index_1 = serial_index >> 8 & 0x00ff;
					can_node_id = node_id;
					data_length = 0;
				}
				...
				const unsigned char *build(const unsigned char *bytes, int &cb)
				{
					...
					bytes += sizeof(uint32_t);
					data_.assign((char*)bytes, data_length);
					return bytes;
				}
...
#pragma pack(pop)
			...
			public:
				...
				template<class T>
				std::shared_ptr<T> shared_for() {			//解析了Udp包
					try {
