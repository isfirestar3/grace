...
namespace gdp
{
	namespace core
	{
		...
		private:
			...
			GdpSingleton(const GdpSingleton&) = delete;
			GdpSingleton(GdpSingleton&&) = delete;
			GdpSingleton& operator=(const GdpSingleton&) = delete;
			GdpSingleton& operator=(GdpSingleton&&) = delete;
		...
		template <class T>
		std::atomic<T*> GdpSingleton<T>::instance_ = { nullptr };

		template <class T>
		std::recursive_mutex GdpSingleton<T>::lock_;

		template <class T>
		int GdpSingleton<T>::associated_ = 0;
	}
}