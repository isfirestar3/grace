...
/**
  ...
  */
/***************************************************************************/
CopleyIO::CopleyIO( Network &net, int16 nodeID )
{
//   IOModule::IOModule( net, nodeID );
	IOModule( net, nodeID );
}
...
/**
  ...
  */
/***************************************************************************/
CopleyIO::CopleyIO( Network &net, int16 nodeID, IOModuleSettings &settings )
{
//   IOModule::IOModule( net, nodeID, settings );
	IOModule( net, nodeID, settings );
}
...
/**
  ...
  These parameters describe the module capabilities.  They 
  are read only.
...
const Error *CopleyIO::GetIOInfo( CopleyIOInfo &info )
{
   ...
   if( !err ) err = sdo.UpldString( CIOOBJID_INFO_MODEL,   0, l=COPLEY_MAX_STRING, info.model    );
   if( !err ) err = sdo.UpldString( CIOOBJID_INFO_MFGINFO, 0, l=COPLEY_MAX_STRING, info.mfgInfo  );
   ...
   if( !err ) err = sdo.UpldString( CIOOBJID_INFO_NAME,    0, l=COPLEY_MAX_STRING, info.name     );
   if( !err ) err = sdo.UpldString( CIOOBJID_INFO_HOSTCFG, 0, l=COPLEY_MAX_STRING, info.hostCfg );
   if( !err ) err = sdo.Upld16( CIOOBJID_INFO_NODECFG,     0, info.nodeID     );
...
/**
  ...
  These parameters describe the modules capabilities.  They 
  are read only.
...
const Error *CopleyIO::GetIODigi( CopleyIODigi &digi )
{
   ...
   for (int i=1; i<=10; i++)
   {
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_BANKMODE,  i, digi.bankMode[i]  );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_PULLUPMSK, i, digi.pullupMsk[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_TYPEMSK,   i, digi.typeMsk[i]   );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_FAULTMSK,  i, digi.faultMsk[i]  );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_INVMSK,    i, digi.invMsk[i]    );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_VALUEMSK,  i, digi.valueMsk[i]  );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_MODEMSK,   i, digi.modeMsk[i]   );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_RAWMSK,    i, digi.rawMsk[i]    );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_HILOMSK,   i, digi.hiLoMsk[i]   );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_LOHIMSK,   i, digi.loHiMsk[i]   );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE0, i, digi.debounce0[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE1, i, digi.debounce1[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE2, i, digi.debounce2[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE3, i, digi.debounce3[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE4, i, digi.debounce4[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE5, i, digi.debounce5[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE6, i, digi.debounce6[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_DIGI_DEBOUNCE7, i, digi.debounce7[i] );
   }
...
const Error *CopleyIO::GetIOAnlg( CopleyIOAnlg &anlg )
{
  const Error *err = 0;
   ...
   for (int i=1; i<=12; i++)
   {
   if( !err ) err = sdo.Upld16( CIOOBJID_ANLG_IRAW,      i, anlg.iRaw[i]      );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_ISCALED,   i, anlg.iScaled[i]   );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_IFACTOR,   i, anlg.iFactor[i]   );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_IOFFSET,   i, anlg.iOffset[i]   );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_IUPLIMIT,  i, anlg.iUpLimit[i]  );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_ILOLIMIT,  i, anlg.iLoLimit[i]  );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_IABSDELTA, i, anlg.iAbsDelta[i] );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_IPOSDELTA, i, anlg.iPosDelta[i] );
   if( !err ) err = sdo.Upld32( CIOOBJID_ANLG_INEGDELTA, i, anlg.iNegDelta[i] );
   if( !err ) err = sdo.Upld16( CIOOBJID_ANLG_IFLAGS,    i, anlg.iFlags[i]    );
   if( !err ) err = sdo.Upld16( CIOOBJID_ANLG_IMASK,     i, anlg.iMask[i]     );
   }
...
const Error *CopleyIO::GetIOPWM( CopleyIOPWM &pwm, CopleyIOInfo &info )
{
   ...
   if( info.fwVersion <23 )
   {
	...
	{
		data[0] = ((i-1) << 8) | (paramID);               //bank concatenated with ParamID
		data[1] = 0;
		ct = 1;

		if( !err ) err = SerialCmd( opcode, ct, max, data );
		pwm.oRaw[i] = ((uint32)data[1] << 16) | (uint32)data[0];
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OSCALED,   i, pwm.oScaled[i]   );
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OFACTOR,   i, pwm.oFactor[i]   );
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OOFFSET,   i, pwm.oOffset[i]   );
	}
   }
   ...
   else
   {
	for (int i=1; i<=12; i++)
	{
		if( !err ) err = sdo.Upld16( CIOOBJID_PWM_ORAW,      i, pwm.oRaw[i]      );
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OSCALED,   i, pwm.oScaled[i]   );
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OFACTOR,   i, pwm.oFactor[i]   );
		if( !err ) err = sdo.Upld32( CIOOBJID_PWM_OOFFSET,   i, pwm.oOffset[i]   );
	}
...
{
   const Error *err = sdo.Download( CIOOBJID_INFO_NAME, 0, COPLEY_MAX_STRING-1, info.name );
   if( !err ) err = sdo.Download( CIOOBJID_INFO_HOSTCFG, 0, COPLEY_MAX_STRING-1, info.hostCfg );
   if( !err ) err = sdo.Dnld16( CIOOBJID_INFO_NODECFG,     0, info.nodeID     );
...
const Error *CopleyIO::SetIODigi( CopleyIODigi &digi )
{
   ...
   for (int i=1; i<=10; i++)
   {
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_BANKMODE,  i, digi.bankMode[i]  );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_PULLUPMSK, i, digi.pullupMsk[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_TYPEMSK,   i, digi.typeMsk[i]   );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_FAULTMSK,  i, digi.faultMsk[i]  );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_INVMSK,    i, digi.invMsk[i]    );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_VALUEMSK,  i, digi.valueMsk[i]  );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_MODEMSK,   i, digi.modeMsk[i]   );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_HILOMSK,   i, digi.hiLoMsk[i]   );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_LOHIMSK,   i, digi.loHiMsk[i]   );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE0, i, digi.debounce0[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE1, i, digi.debounce1[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE2, i, digi.debounce2[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE3, i, digi.debounce3[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE4, i, digi.debounce4[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE5, i, digi.debounce5[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE6, i, digi.debounce6[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_DIGI_DEBOUNCE7, i, digi.debounce7[i] );
   }
...
const Error *CopleyIO::SetIOAnlg( CopleyIOAnlg &anlg )
{
   ...
   for (int i=1; i<=12; i++)
   {
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_IFACTOR,   i, anlg.iFactor[i]   );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_IOFFSET,   i, anlg.iOffset[i]   );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_IUPLIMIT,  i, anlg.iUpLimit[i]  );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_ILOLIMIT,  i, anlg.iLoLimit[i]  );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_IABSDELTA, i, anlg.iAbsDelta[i] );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_IPOSDELTA, i, anlg.iPosDelta[i] );
   if( !err ) err = sdo.Dnld32( CIOOBJID_ANLG_INEGDELTA, i, anlg.iNegDelta[i] );
   if( !err ) err = sdo.Dnld16( CIOOBJID_ANLG_IMASK,     i, anlg.iMask[i]     );
   }
...
const Error *CopleyIO::SetIOPWM( CopleyIOPWM &pwm, CopleyIOInfo &info )
{
   ...
   if( info.fwVersion < 23 )
   {
	  ...
	  {		  
		...
		if( !err ) err = SerialCmd( opcode, ct, max, data );
		if( !err ) err = sdo.Dnld32( CIOOBJID_PWM_OFACTOR,   i, pwm.oFactor[i]   );
		if( !err ) err = sdo.Dnld32( CIOOBJID_PWM_OOFFSET,   i, pwm.oOffset[i]   );
	  }
   }
   ...
   else
   {
	  for (uint16 i=1; i<=12; i++)
	  {
		if( !err ) err = sdo.Dnld32( CIOOBJID_PWM_OSCALED,   i, pwm.oScaled[i]   );
		if( !err ) err = sdo.Dnld32( CIOOBJID_PWM_OFACTOR,   i, pwm.oFactor[i]   );
		if( !err ) err = sdo.Dnld32( CIOOBJID_PWM_OOFFSET,   i, pwm.oOffset[i]   );
	  }
   }
