...
*/
/***************************************************************************/
const Error *VectorCAN::RecvFrame( CanFrame &frame, Timeout timeout )
{
   ...
   int32 msTimeout = (int32)timeout;

   if( timeout < 0 ) msTimeout = INFINITE;
   ...
   while( 1 )
   {
	 ...
	 const Error *eptr = WaitOnWindowsObject( ldat->event, msTimeout );
	 if( eptr ) return eptr;
...
*/
/***************************************************************************/
const Error *VectorCAN::XmitFrame( CanFrame &frame, Timeout timeout )
{
   LocalData *ldat = (LocalData *)data;
