...
typedef canStatus(CANLIBAPI *canWriteWaitType)(int, long, void *, uint, uint, ulong);
typedef canStatus(CANLIBAPI *canWriteSyncType)(int, ulong);
...
static canWriteWaitType LPcanWriteWait;
static canWriteSyncType LPcanWriteSync;
...
const Error *KvaserCAN::SetBaud(int32 b) {
    switch (b) {
        case 1000000: kvBaud = BAUD_1M;
            break;
        case 500000: kvBaud = BAUD_500K;
            break;
        case 250000: kvBaud = BAUD_250K;
            break;
        case 125000: kvBaud = BAUD_125K;
            break;
        case 100000: kvBaud = BAUD_100K;
            break;
        case 62000: kvBaud = BAUD_62K;
            break;
        case 50000: kvBaud = BAUD_50K;
            break;
        default:
...
const Error *KvaserCAN::RecvFrame(CanFrame &frame, Timeout timeout) {
    ...
    if (timeout == 0) {
        IncReadCount(1);
        try {
            status = LPcanRead(Handle_Rd, &id, frame.data, &dlc, &flags, &time);
            frame.id = id & 0xFFFFFFFF;
        ...
        while (timeout) {
            ...
            try {
               // status = LPcanReadWait(Handle_Rd, (long*) &frame.id, frame.data, &dlc, &flags, &time, to);
                 status = LPcanReadWait(Handle_Rd, &id, frame.data, &dlc, &flags, &time, to);
                frame.id = id & 0xFFFFFFFF;
            } catch (...) {
                IncReadCount(-1);
                throw;
...
const Error *KvaserCAN::XmitFrame(CanFrame &frame, Timeout timeout) {
    ...
    // 如果超时为0， 不能保证内核行为， 因此必须显式调用数据同步完成
//    if ((timeout >= -EPSINON) && (timeout <= EPSINON)) {;
//        status = LPcanWrite(Handle_Wr, id, frame.data, dlc, flags);
//        if (canOK == status) {
//            status = LPcanWriteSync(Handle_Wr, 0xFFFFFFFF);
//        }
//    } else {
//        status = LPcanWriteWait(Handle_Wr, id, frame.data, dlc, flags, (ulong) timeout);
//    }
    status = LPcanWrite(Handle_Wr, id, frame.data, dlc, flags);
    mutex.Unlock();
...
static const Error *InitLibrary(void) {
    ...
    if (!openCards) {
        ...
        if (!hDLL) {
            ...
            LPcanWriteWait = (canWriteWaitType) GetProcAddress(hDLL, "canWriteWait");
            LPcanWriteSync = (canWriteSyncType) GetProcAddress(hDLL, "canWriteSync");

            if (!LPcanBusOn || !LPcanSetBusParams || !LPcanIoCtl ||
                    !LPcanOpenChannel || !LPcanInitLibrary || !LPcanClose ||
                    !LPcanBusOff || !LPcanReadWait || !LPcanRead || !LPcanWrite || !LPcanWriteWait || !LPcanWriteSync) {
                err = &CanError::NoDriver;
                FreeLibrary(hDLL);
            }
        }
