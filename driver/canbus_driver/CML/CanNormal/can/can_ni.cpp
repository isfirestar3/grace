...
*/
/***************************************************************************/
const Error *NI_Can::RecvFrame( CanFrame &frame, Timeout timeout )
{
	NCTYPE_STATE state;
	int32 ncTO = (int32)timeout;

	if( ncTO == 0 ) 
		ncTO = NC_DURATION_NONE;
	else if( ncTO < 0 )
		ncTO = NC_DURATION_INFINITE;
...
*/
/***************************************************************************/
const Error *NI_Can::XmitFrame( CanFrame &frame, Timeout timeout )
{
...
//	printf( "\n" );
	...
	while( 1 )
	{
		...
		Timeout delay = (timeout > 0 && timeout < 5) ? timeout : 5;
		Thread::sleep( delay );
