...
typedef HRESULT ( VCIAPI *vciEnumDeviceOpenType		) 
	( PHANDLE );
typedef HRESULT ( VCIAPI *vciEnumDeviceNextType		) 
	( HANDLE, PVCIDEVICEINFO );
typedef HRESULT ( VCIAPI *vciEnumDeviceCloseType	) 
	( HANDLE );
typedef HRESULT ( VCIAPI *canControlStartType		) 
	( HANDLE, BOOL );
typedef HRESULT ( VCIAPI *canChannelActivateType	) 
	( HANDLE, BOOL );
typedef HRESULT ( VCIAPI *canControlOpenType		) 
	( HANDLE, UINT32, PHANDLE );
typedef HRESULT ( VCIAPI *canControlInitializeType	) 
	( HANDLE, UINT8, UINT8, UINT8 );
typedef HRESULT ( VCIAPI *canChannelOpenType		) 
	( HANDLE, UINT32, BOOL, PHANDLE );
typedef HRESULT ( VCIAPI *canChannelCloseType		) 
	( HANDLE );
typedef HRESULT ( VCIAPI *canControlCloseType		) 
	( HANDLE );
typedef HRESULT ( VCIAPI *vciDeviceOpenType			) 
	( REFVCIID, PHANDLE );
typedef HRESULT ( VCIAPI *vciDeviceCloseType		) 
	( HANDLE );
typedef HRESULT ( VCIAPI *canChannelSendMessageType	) 
	( HANDLE, UINT32, PCANMSG );
typedef HRESULT ( VCIAPI *canChannelReadMessageType	) 
	( HANDLE, UINT32, PCANMSG );
typedef HRESULT ( VCIAPI *canControlSetAccFilterType) 
	( HANDLE, BOOL, UINT32, UINT32 );
typedef HRESULT ( VCIAPI *canChannelInitializeType	) 
	( HANDLE, UINT16, UINT16, UINT16, UINT16 );
typedef HRESULT ( VCIAPI *canControlResetType		) 
	( HANDLE );
...
const Error *IxxatCANV3::RecvFrame( CanFrame &frame, Timeout timeout )
{
	...
	hResult = lp_canChannelReadMessage( hCanChn, 100, &sCanMsg);
...
static const Error *InitLibrary( void )
{
   ...
   if( !openCards )
   {
	  ...
	  if( !hVCINPL )
	  {
		...
		lp_vciEnumDeviceOpen	= (vciEnumDeviceOpenType)	
			GetProcAddress( hVCINPL, "vciEnumDeviceOpen"		);
		lp_vciEnumDeviceNext	= (vciEnumDeviceNextType)
			GetProcAddress( hVCINPL, "vciEnumDeviceNext"		);
		lp_vciEnumDeviceClose	= (vciEnumDeviceCloseType)
			GetProcAddress( hVCINPL, "vciEnumDeviceClose"		);
		lp_canControlStart	= (canControlStartType)			
			GetProcAddress( hVCINPL, "canControlStart"			);
		lp_canChannelActivate	= (canChannelActivateType)		
			GetProcAddress( hVCINPL, "canChannelActivate"		);
		lp_canControlOpen	= (canControlOpenType)	
			GetProcAddress( hVCINPL, "canControlOpen"			);
		lp_canControlInitialize	= (canControlInitializeType)	
			GetProcAddress( hVCINPL, "canControlInitialize"		);	
		lp_canChannelClose	= (canChannelCloseType)			
			GetProcAddress( hVCINPL, "canChannelClose"			);
		lp_canChannelOpen	= (canChannelOpenType)			
			GetProcAddress( hVCINPL, "canChannelOpen"			);
		lp_canControlClose	= (canControlCloseType)			
			GetProcAddress( hVCINPL, "canControlClose"			);
		lp_vciDeviceOpen		= (vciDeviceOpenType)		
			GetProcAddress( hVCINPL, "vciDeviceOpen"			);
		lp_vciDeviceClose		= (vciDeviceCloseType)			
			GetProcAddress( hVCINPL, "vciDeviceClose"			);
		lp_canChannelSendMessage	= (canChannelSendMessageType)	
			GetProcAddress( hVCINPL, "canChannelSendMessage"	);
		lp_canChannelReadMessage	= (canChannelReadMessageType)	
			GetProcAddress( hVCINPL, "canChannelReadMessage"	);
		lp_canControlSetAccFilter	= (canControlSetAccFilterType)
			GetProcAddress( hVCINPL, "canControlSetAccFilter"	);
		lp_canChannelInitialize		= (canChannelInitializeType)
			GetProcAddress( hVCINPL, "canChannelInitialize"		);
		lp_canControlReset	= (canControlResetType)			
			GetProcAddress( hVCINPL, "canControlReset"		);
		
		if( !lp_canChannelSendMessage || !lp_vciEnumDeviceOpen 	    || 
			!lp_vciEnumDeviceOpen 	  || !lp_vciEnumDeviceOpen 	    || 	
			!lp_canControlOpen 		  || !lp_canControlInitialize   || 
			!lp_canChannelClose 	  || !lp_canControlClose 	    || 
			!lp_vciDeviceClose 		  || !lp_vciDeviceOpen 		    ||  
			!lp_canControlStart 	  || !lp_canChannelActivate     || 
			!lp_canChannelInitialize  || !lp_canControlSetAccFilter ||
			!lp_canControlReset		  || !lp_canChannelReadMessage )
		{
			err = &CanError::NoDriver;
			FreeLibrary( hVCINPL );
		}	
