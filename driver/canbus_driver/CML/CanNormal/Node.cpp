...
const Error *Node::UnInit( void )
{
	...
	if( netRef )
	{
		cml.Debug( "Node: %d uninitializing\n", nodeID );
...
const Error *Node::SynchStart( void )
{
	...
	RefObjLocker<CanOpen> co( netRef );
	...
	/*
	@Seven Elmo Can not set id to 40000080
	uint32 id;
	const Error *err = GetSynchId( id );
	if( !err )
		err = SetSynchId( id | 0x40000000 );
 ...
 */
/***************************************************************************/
const Error *Node::StartHeartbeat( uint16 period, byte timeout )
{
	RefObjLocker<Network> net( netRef );
	if( !net ) return &NodeError::NetworkUnavailable;
	return net->SetNodeGuard( this, GUARDTYPE_HEARTBEAT, period, timeout );
}
