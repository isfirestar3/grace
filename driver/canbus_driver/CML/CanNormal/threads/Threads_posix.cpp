...
{
	data = new semaphore_t;

	if( data )
		semaphore_create(mach_task_self(), (semaphore_t *)data, SYNC_POLICY_FIFO, count);
	else
...
//		struct timespec ts;
		...
		if (err != KERN_SUCCESS) return &ThreadError::General;
		err = clock_get_time(cclock, &mts);
		if (err != KERN_SUCCESS) return &ThreadError::General;
		err = mach_port_deallocate(mach_task_self(), cclock);
		if (err != KERN_SUCCESS) return &ThreadError::General;
//		ts.tv_sec = mts.tv_sec;
//		ts.tv_nsec = mts.tv_nsec;

		to += (mts.tv_nsec / 1000000);
		long sec = to / 1000;
		to -= sec * 1000;

		mts.tv_sec += sec;
		mts.tv_nsec = to * 1000000;

        err = semaphore_timedwait(*(semaphore_t *)data, mts);
        if(err != KERN_SUCCESS)
        	return &ThreadError::General;
	}
	else
	{
		err =  semaphore_wait(*(semaphore_t *)data);
	}
...
const Error *Semaphore::Put( void )
{
	...
	if( semaphore_signal(*(semaphore_t *)data))
		return &ThreadError::General;
