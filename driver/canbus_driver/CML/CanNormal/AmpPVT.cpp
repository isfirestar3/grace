...
const Error *Amp::PvtWriteBuff( uint8 *buff, bool viaSDO )
{
   if( viaSDO || (GetNetworkType() == NET_TYPE_ETHERCAT) )
      return sdo.Download( OBJID_PVT_DATA, 0, 8, buff );
   else
      return pvtPdo.Transmit( buff );
}
...
/**
  ...
  */
/***************************************************************************/
void TPDO_PvtBuffStat::Received( void )
{
   newStat = stat.Read();

   if( !first && (newStat==prev) )
      return;
...
void TPDO_PvtBuffStat::run( void )
{
   while( 1 )
   {
	 ...
	 continue;
      ...
      ampPtr->PvtStatusUpdate( newStat );
   }
}
...
const Error *RPDO_Pvt::Init( Amp &amp, uint16 slot )
{
   ...
   const Error *err = 0; //RPDO::Init( amp.GetRpdoCobID(slot) );
