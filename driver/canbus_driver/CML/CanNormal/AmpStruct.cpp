...
const Error *Amp::GetMtrInfo( MtrInfo &info )
{
   ...
   if( !err && (ct >=33) && CheckFeature( FEATURE_GEAR_RATIO ) )
      err = sdo.Upld32( OBJID_MOTOR_INFO, 33, info.gearRatio );

   if( !err && (ct >=34) && CheckFeature( FEATURE_RESOLVER_CYCLES ) )
      err = sdo.Upld16( OBJID_MOTOR_INFO, 34, info.resolverCycles );
   ...
   if( !err && CheckFeature( FEATURE_ENC_OPTIONS ) )
		err = sdo.Upld32( OBJID_MTR_ENC_OPTIONS, 0, info.mtrEncOptions );
      
   if( !err && CheckFeature( FEATURE_ENC_OPTIONS ) )
		err = sdo.Upld32( OBJID_LOAD_ENC_OPTIONS, 0, info.loadEncOptions );
...
const Error *Amp::SetMtrInfo( MtrInfo &info )
{
   ...
   if( !err && (ct >=33) && CheckFeature( FEATURE_GEAR_RATIO ) )
      err = sdo.Dnld32( OBJID_MOTOR_INFO, 33, info.gearRatio );

   if( !err && (ct >=34) && CheckFeature( FEATURE_RESOLVER_CYCLES ) )
      err = sdo.Dnld16( OBJID_MOTOR_INFO, 34, info.resolverCycles );

   if( !err && CheckFeature( FEATURE_HALL_VEL_SHIFT ) )
      err = sdo.Dnld16( OBJID_HALLVEL_SHIFT, 0, info.hallVelShift );

   if( !err && CheckFeature( FEATURE_ENC_OPTIONS ) )
		err = sdo.Dnld32( OBJID_MTR_ENC_OPTIONS, 0, info.mtrEncOptions );
      
   if( !err && CheckFeature( FEATURE_ENC_OPTIONS ) )
		err = sdo.Dnld32( OBJID_LOAD_ENC_OPTIONS, 0, info.loadEncOptions );
...
const Error *Amp::GetVelLoopConfig( VelLoopConfig &cfg )
{
   ...
   if( !err && CheckFeature( FEATURE_VI_DRAIN ) )
	  err = sdo.Upld16( OBJID_VELLOOP, 5, cfg.viDrain );
...
const Error *Amp::SetVelLoopConfig( VelLoopConfig &cfg )
{
   ...
   if( !err ) err = sdo.Dnld16( OBJID_VELLOOP, 4, cfg.shift );
       ...
       /// viDrain only in some firmware versions
   if( !err && CheckFeature( FEATURE_VI_DRAIN ) )
	 err = sdo.Dnld16( OBJID_VELLOOP, 5, cfg.viDrain );
...
const Error *Amp::SetSoftLimits( SoftPosLimit &cfg )
{
   ...
   {  
	   err = sdo.Dnld32( OBJID_MOTOR_WRAP, 0, m );
	   err = sdo.Dnld32( OBJID_LOAD_WRAP, 0, l );
   }
...
const Error *Amp::GetSoftLimits( SoftPosLimit &cfg )
{
   ...
   if( !err && CheckFeature( FEATURE_POS_WRAP ) )
   {  
	   err = sdo.Upld32( OBJID_MOTOR_WRAP, 0, m );
	   err = sdo.Upld32( OBJID_LOAD_WRAP, 0, l );
   }
	...
	if(CheckFeature( FEATURE_USTEP_OUTER_LOOP ) )
	 {
		...
		err = sdo.Dnld32( OBJID_USTEP_MAX_VEL_ADJ, 0, cfg.maxVelAdj );
		if( !err ) err = sdo.Dnld16( OBJID_USTEP_P_GAIN_OUT_LOOP, 0, cfg.ustepPGainOutLoop );
	}

	if( !err && CheckFeature( FEATURE_STEP_DETENT_GAIN ) )
		 err = sdo.Dnld16( OBJID_USTEP_DETENT_COR_GAIN,  0, cfg.detentCorrectionGain );


	if( !err && CheckFeature( FEATURE_USTEP_CONFIG_STATUS ) )
	    err = sdo.Dnld16( OBJID_USTEP_CONFIG_STAT, 0, cfg.ustepConfigAndStatus );
...
const Error *Amp::GetUstepConfig( UstepConfig &cfg )
{
	...
	const Error *err = 0;
	 ...
	 if(CheckFeature( FEATURE_USTEP_OUTER_LOOP ) )
	 {
		  err = sdo.Upld32( OBJID_USTEP_MAX_VEL_ADJ, 0, cfg.maxVelAdj );
		  if( !err ) err = sdo.Upld16( OBJID_USTEP_P_GAIN_OUT_LOOP, 0, cfg.ustepPGainOutLoop );
	 }
	  

     if( !err && CheckFeature( FEATURE_STEP_DETENT_GAIN ) )
      err = sdo.Upld16( OBJID_USTEP_DETENT_COR_GAIN,  0, cfg.detentCorrectionGain );

     if( !err && CheckFeature( FEATURE_USTEP_CONFIG_STATUS ) )
	  err = sdo.Upld16( OBJID_USTEP_CONFIG_STAT, 0, cfg.ustepConfigAndStatus );
	...
	if(CheckFeature( FEATURE_ALGO_PHASE_INIT_CUR ) )
	 {
		...
		err = sdo.Dnld16( OBJID_APHASE_INIT_CUR, 0, cfg.phaseInitCurrent );
		if( !err ) err = sdo.Dnld16( OBJID_APHASE_INIT_TIME, 0, cfg.phaseInitTime );
	}

	if( !err && CheckFeature( FEATURE_ALGO_PHASE_INIT_CONFIG ) )
	    err = sdo.Dnld16( OBJID_APHASE_INIT_CONFIG, 0, cfg.phaseInitConfig );
...
const Error *Amp::GetAlgoPhaseInit( AlgoPhaseInit &cfg )
{
	...
	const Error *err = 0;
	 ...
	 if(CheckFeature( FEATURE_ALGO_PHASE_INIT_CUR ) )
	 {
		...
		err = sdo.Upld16( OBJID_APHASE_INIT_CUR, 0, cfg.phaseInitCurrent );
		if( !err ) err = sdo.Upld16( OBJID_APHASE_INIT_TIME, 0, cfg.phaseInitTime );
	}

	if( !err && CheckFeature( FEATURE_ALGO_PHASE_INIT_CONFIG ) )
	    err = sdo.Upld16( OBJID_APHASE_INIT_CONFIG, 0, cfg.phaseInitCurrent );
...
const Error *Amp::SetCammingConfig( CammingConfig &cfg )
{
	...
	err = sdo.Dnld16( OBJID_CAM_CONFIG, 0, cfg.cammingModeConfig );
	if( !err ) err = sdo.Dnld16( OBJID_CAM_DELAYF, 0, cfg.cammingDelayForward );
	if( !err ) err = sdo.Dnld16( OBJID_CAM_DELAYR,  0, cfg.cammingDelayReverse );
	if( !err ) err = sdo.Dnld32( OBJID_CAM_VEL, 0, cfg.cammingMasterVel );
...
const Error *Amp::GetCammingConfig( CammingConfig &cfg )
{
	...
	if(!CheckFeature( FEATURE_CAMMING ) )
	{
  	...
  	err = sdo.Upld16( OBJID_CAM_CONFIG, 0, cfg.cammingModeConfig );
	if( !err ) err = sdo.Upld16( OBJID_CAM_DELAYF, 0, cfg.cammingDelayForward );
	if( !err ) err = sdo.Upld16( OBJID_CAM_DELAYR,  0, cfg.cammingDelayReverse );
	if( !err ) err = sdo.Upld32( OBJID_CAM_VEL, 0, cfg.cammingMasterVel );
...
const Error *Amp::SetGainScheduling( GainScheduling &cfg )
{
	...
	err = sdo.Dnld32( OBJID_GAIN_SCH_CONFIG, 0, cfg.gainSchedulingConfig );
...
const Error *Amp::GetGainScheduling( GainScheduling &cfg )
{
	...
	if(!CheckFeature( FEATURE_GAIN_SCHED ) )
	{
  	...
  	err = sdo.Upld32( OBJID_GAIN_SCH_CONFIG, 0, cfg.gainSchedulingConfig );
...
const Error *Amp::SetCanNetworkConfig( CanNetworkConfig &cfg )
{
   ...
   const Error *err = sdo.Dnld16( OBJID_CANID_CFG, 0, value );


   if( !err && CheckFeature( FEATURE_PIN_MAP ) )
	err = sdo.Dnld32( OBJID_CANID_PIN_MAP, 0, cfg.pinMapping );

   if( !err && CheckFeature( FEATURE_CAN_SETTINGS ) )
   {
	   err = sdo.Dnld16( OBJID_CAN_HARTBEAT_TIME, 0, cfg.heartbeat );
	   if( !err ) err = sdo.Dnld16( OBJID_CAN_NODEGUARD_TIME, 0, cfg.nodeGuard );	   
	   if( !err ) err = sdo.Dnld8( OBJID_CAN_NODEGUARD_LIFE, 0, a );
   }

   if( !err && CheckFeature( FEATURE_CAN_OPTIONS ) )
   {
	   err = sdo.Dnld16( OBJID_CAN_QSTOP, 0, cfg.quickStop );
	   if( !err ) err = sdo.Dnld16( OBJID_CAN_SHUT_DOWN, 0, cfg.shutDownOption );
	   if( !err ) err = sdo.Dnld16( OBJID_CAN_DISABLE, 0, cfg.disableOption );
	   if( !err ) err = sdo.Dnld16( OBJID_CAN_HALT, 0, cfg.haltOption );
   }
...
const Error *Amp::GetCanNetworkConfig( CanNetworkConfig &cfg )
{
   ...
   if( !err && CheckFeature( FEATURE_PIN_MAP ) )
	err = sdo.Upld32( OBJID_CANID_PIN_MAP, 0, cfg.pinMapping );

   if( !err && CheckFeature( FEATURE_CAN_SETTINGS ) )
   {
	   uint8 a;
	   err = sdo.Upld16( OBJID_CAN_HARTBEAT_TIME, 0, cfg.heartbeat );
	   if( !err ) err = sdo.Upld16( OBJID_CAN_NODEGUARD_TIME, 0, cfg.nodeGuard ); 
	   if( !err ) err = sdo.Upld8( OBJID_CAN_NODEGUARD_LIFE, 0, a );
		cfg.nodeGuardLife = (uint16)a;
   }

   if( !err && CheckFeature( FEATURE_CAN_OPTIONS ) )
   {
	   err = sdo.Upld16( OBJID_CAN_QSTOP, 0, cfg.quickStop );
	   if( !err ) err = sdo.Upld16( OBJID_CAN_SHUT_DOWN, 0, cfg.shutDownOption );
	   if( !err ) err = sdo.Upld16( OBJID_CAN_DISABLE, 0, cfg.disableOption );
	   if( !err ) err = sdo.Upld16( OBJID_CAN_HALT, 0, cfg.haltOption );
   }
   if( err ) return err;
