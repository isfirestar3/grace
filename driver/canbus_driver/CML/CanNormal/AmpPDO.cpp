...
const Error *RPDO_Ctrl::Init( Amp &amp, uint16 slot, uint16 id )
{
	ampPtr = &amp;
	ctrl.Write( amp.lastCtrlWord );

	const Error *err = RPDO::Init( id );

	if( !err ) err = ctrl.Init( OBJID_CONTROL, 0 );
...
const Error *RPDO_Ctrl::Transmit( uint16 val )
{
	// Quit on CANopen networks where this isn't used.
	if( !ampPtr ) return 0;
