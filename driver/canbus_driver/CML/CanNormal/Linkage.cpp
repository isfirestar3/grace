...
const Error *Linkage::Init( uint16 ct, Amp *a[] )
{
	...
	// the same network.
	int i;
	uint32 netRef = a[0]->GetNetworkRef();
	...
	if( amp[0]->GetNetworkType() == NET_TYPE_CANOPEN )
		return ctrlPDO.Init();
	else
		return 0;
}
...
const Error *Linkage::SetControlWord( uint16 value )
{
	...
	else
	{
		...
		RefObjLocker<EtherCAT> ecat( amp[0]->GetNetworkRef() );
		{
...
const Error *RPDO_LinkCtrl::Transmit( uint16 c )
{
	...
	RefObjLocker<Network> net( link[0].GetNetworkRef() );
