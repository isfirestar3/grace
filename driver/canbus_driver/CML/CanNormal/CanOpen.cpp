...
CML_NEW_ERROR( CanOpenError, BadDeviceID, 	  "The specified Device ID was invalid" );
CML_NEW_ERROR( CanOpenError, DeviceIDUsed,    "A device with the specified ID is already present on the network" );
...
const Error *CanOpen::XmitSDO( Node *n, uint8 *buff, uint16 len, uint16 *ret, Timeout timeout )
{
	...
	*ret = 8;
	...
	// FIXME: if timeout of Xmit is 1000, copley will be very slow,
	//		  Don't know why! Simon CHENG
	const Error *err = Xmit( frame, 5 );
...
struct CoTpdoInfo: public Receiver
{
	...
	int NewFrame( CanFrame &frame )
	{
		...
		if( pdo ) pdo->ProcessData( frame.data, frame.length );
...
const Error *CanOpen::PdoEnable( Node *node, uint16 slot, PDO *pdo )
{
	...
	// FIXME: This step is not necessary in some situation. Simon CHENG
...
{
	//FIXME: Not sure if the following code is necessary! Simon CHENG
	/// Disable this TPDO's receiver, otherwise cannot remap. Add by Simon CHENG.
	if( pdo->IsTxPDO() &&  (int32)pdo->GetID()>0x180)
	{
		const Error *err;
		err = DisableReceiver( pdo->GetID() );
		if(err)
			return err;
		else
			printf("disable pdo receiver: %d\n",  pdo->GetID());
	}
	printf("disable pdo: %d\n",  pdo->GetID());
...
const Error *CanOpen::PdoDisable( Node *node, uint16 slot, bool isTxPDO )
{
	...
	NodeIdentity identify_id;
	//cml.Debug( "Amp %d, checking ID\n", nodeID );
	err =node->GetIdentity( identify_id );
	if( err ) return err;

	if (identify_id.vendorID!=0x9A)
	{
		//Copley
		if( !(id & 0x80000000) )
