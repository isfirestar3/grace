...
// @param buff  Points to the frame buffer
...
public:
   ...
   const Error *NewData( void )
   {
      ...
      for( int i=0; i<ct; i++ )
      {
         ...
         if( pdo )
            pdo->ProcessData( datPtr, pdos[i].byteLen );
...
public:
   ...
   void Freshen( EtherCAT *ecat )
   {
      uint8 *ptr = buff;
      for( int i=0; i<pdos.length(); i++ )
      {
         ecat->LoadPdoDat( pdos[i].ref, ptr, pdos[i].byteLen );
         ptr += pdos[i].byteLen;
      }
   }
};
...
const Error *EtherCAT::ResetNode( Node *n )
{
   ...
   for( int i=0; i<3; i++ )
   {
      const Error *err = NodeWrite( n, 0x40, 1, &buff[i] );
      if( err ) return err;
   }
   return 0;
}
...
const Error *EtherCAT::SetSync0Period( Node *n, uint32 ns )
{
   ...
   // I shout for about 10ms worth initially
...
cml.Debug( "Sys: %lld, next: %lld, diff: %lld\n", sysTime, nxtTime, nxtTime-sysTime );
...
void EtherCAT::ReadThreadFunc( void )
{
   ...
   while( !stopSem )
   {
	 ...
	 continue;
      ...
      if( !frame )
      {
         cml.Debug( "EtherCAT::ReadThreadFunc - Unable to identify received frame\n" );
         continue;
      }
   ...
   while( !stopSem )
   {
	 ...
	 if( !ni )
	 {
	    ...
	    continue;
         ...
         if( ni->rpdos.isEnabled() )
         {
            ni->rpdos.Freshen( this );
...
const Error *EtherCAT::MailboxTransfer( Node *n, uint16 len, uint16 *ret, Timeout timeout )
{
   ...
   // FIXME: Need to rethink the max retry logic here.  I should really be retrying based on the mailbox counter
   int maxRetry = 3;
   while( maxRetry-- )
   {
      EcatFrame frame;
	 ...
	 *ret = retLen;
	 ...
	 if( (retLen >= 6) && (resp[5]==0) )
	 {
	    if( retLen < 10 ) return &EtherCatError::MboxError;

	    switch( bytes_to_int16( &resp[8] ) )
	    {
	       ...
	       default: return &EtherCatError::MboxError;
	    }
	 }

	 return 0;
      }
      while( (Thread::getTimeMS() - startTime) < timeout );
   }
...
void EtherCAT::LoadPdoDat( uint32 ref, uint8 *buff, int max )
{
   RefObjLocker<RPDO> pdo( ref );
   int ct = pdo ? pdo->netBuffLen : 0;
   if( ct > max ) ct = max;
...
const Error *EtherCAT::FoE_DnldStart( Node *n, const char *filename, uint32 password, Timeout to )
{
   ...
   delete ni->foeBuffer;
   cml.Debug( "foeBuffer size %d\n", ni->syncLen[SM_RXMBX]-12 );
   ni->foeBuffer = new uint8[ ni->syncLen[SM_RXMBX]-12 ];
   ni->foeRemain = 0;
...
/**
  ...
  @param buff  Buffer holding the data to download
