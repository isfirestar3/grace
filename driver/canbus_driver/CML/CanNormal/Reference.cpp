...
RefObj::RefObj( const char *name )
{
   ...
   else if( !remainInBlock )
   {
	 ...
	 return;
      ...
      if( !refBlocks[totAllocBlocks] ) return;
...
@param id The reference previously returned by a call to RefObj::GrabRef
 */
/***************************************************************************/
...
@param id The reference ID associated with the object
