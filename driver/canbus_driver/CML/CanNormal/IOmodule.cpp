...
const Error *IOModule::Init( Network &net, int16 nodeID, IOModuleSettings &settings )
{
   ...
   if( settings.useStandardAoutPDO )
   {
	 ...
	 err = aoutPDO[n].Init( this, map, ids );
   ...
   if( ct && settings.useStandardDinPDO )
   {
      ...
      err = dinPDO.Init( this, ct, ids, IOEVENT_DIN_PDO0 );
   ...
   if( settings.useStandardAinPDO )
   {
	 ...
	 err = ainPDO[n].Init( this, map, ids, (IOMODULE_EVENTS)(IOEVENT_AIN_PDO0<<n) );
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::DigOutPDO::Init( IOModule *io, uint8 ct, uint8 id[] )
{
   if( ct < 1 || ct > 8 )
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::AlgOutPDO::Init( class IOModule *io, uint8 ct, uint8 id[] )
{
   if( ct < 1 || ct > 4 )
      return &IOError::BadIOCount;
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::DigInPDO::Init( class IOModule *io, uint8 ct, uint8 id[], IOMODULE_EVENTS event )
{
   if( ct < 1 || ct > 8 )
      return &IOError::BadIOCount;
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::AlgInPDO::Init( class IOModule *io, uint8 ct, uint8 id[], IOMODULE_EVENTS event )
{
   if( ct < 1 || ct > 4 )
      return &IOError::BadIOCount;
