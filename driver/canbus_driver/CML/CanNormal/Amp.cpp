...
const Error *Amp::Init( Network &net, int16 nodeID, AmpSettings &settings )
{
	...
	// Initialize a receive PDO to send data to the drive.
	// The type of PDO used depends on the network type
	if( GetNetworkType() == NET_TYPE_ETHERCAT )
		err = ctrlPdo.Init( *this, 0, 0 );
	else
		err = pvtPdo.Init( *this, 0 );
	if( err ) goto retErr;
	...
	// For EtherCAT, set the SYNC0 period
	else
	{
...
const Error *Amp::WaitHomeDone( Timeout timeout )
{
	...
	// a homing error.
	uint16 stat = statPdo.status.Read();
