...
int canbus_manager::canio__create_canbus(const _canbus_argv *can, int baud) {
    ...
    int key = (can->type_ & 0xFFFF) << 16 + can->port_;
...
//
//int canbus_manager::canio__write( const _canbus_argv * can, var__functional_object_t **vars, int cnt ) {
...
//}
//
//int canbus_manager::canio__read( const _canbus_argv * can, var__functional_object_t **vars, int cnt ) {
//	if (nullptr == can || nullptr == vars || cnt == 0)
//	{
//		return -1;
//	}
    ...
    for (int i = 0; i < cnt; ++i) {
        ...
        if (itr_bus == map_dirver_.end()) {
            ...
            if (itr_drv == itr_bus->second.end()) {
                ...
                itr_drv->second->read_and_write(vars[i]);
            }
        }
    }
