...
#ifdef _DEBUG
		lowarn("operation_net") << "can not push new asio_block into map,there is already exist pkt_id.";
#endif
		return -1;
	}
...
int asio_manage::exec(const int pkt_id, const void*data){
...
#ifdef _DEBUG
			loerror("operation_net") << "can not operate exec function,can not find id:" << pkt_id << " package.";
#endif
			return -1;
		}
	...
	if (asio->callback_function_){
		loinfo("operation_net") << "success receive pkt is:" << pkt_id;
		asio->callback_function_(data);
		return 0;
	}
#ifdef _DEBUG
	loerror("operation_net") << "can not callback fuction,there isn't callback_function_ in asio_block.";
#endif
	return -1;
}
...
#ifdef _DEBUG
	loerror("operation_net") << "can not callback fuction,there isn't object_callback_function_ in asio_block.";
#endif
	return -1;
}
...
int asio_manage::remove(const int pkt_id, std::shared_ptr<motion::asio_block>&asio){
	...
	if (iter == asio_map_.end()){
#ifdef _DEBUG
		loerror("operation_net") << "can not remove in asio,can not find id:" << pkt_id << " package.";
#endif
		return -1;
	}
	asio = iter->second;
...
int asio_manage::write(const int pkt_id, const std::shared_ptr<motion::asio_block>&asio, const std::function<int()>&MyWrite){
	if (!MyWrite){
#ifdef _DEBUG
		loerror("operation_net") << "lamba function is not exist.";
#endif
		return -1;
	}
...
#ifdef _DEBUG
		loerror("operation_net") << "failed to send packeage,the pkt_id is " << pkt_id;
#endif
		return -1;
	}
	loinfo("operation_net") << "send package id:" << pkt_id;
	return 0;
}

void asio_manage::timeout_check(){
	static const uint32_t ASIO_CHECK_INTERVAL = 3000;
	static const uint32_t ASIO_TIMEOUT = 3000;
	uint32_t tick;
...
#ifdef _DEBUG
							lowarn("operation_net") << "asio manage get a timeout,the pkt_id is "<<iter->first;
#endif
							//回调上层
...
#ifdef _DEBUG
						lowarn("operation_net") << "asio manage get a overflow timeout,the pkt_id is " << iter->first;
#endif
						//回调上层
						iter->second->error_ = nsp::proto::errorno_t::kRequestTimeout;
						timeout_notify(iter->second);
